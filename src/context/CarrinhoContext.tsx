...
interface CarrinhoContextProps {
  itens: ItemCarrinho[];
  adicionar: (produto: Produto, corSelecionada?: Cor, quantidade?: number) => void;
  remover: (id: number, corSelecionada?: Cor) => void;
  atualizarQuantidade: (id: number, quantidade: number, corSelecionada?: Cor) => void;
  limpar: () => void;
}
...
export function CarrinhoProvider({ children }: { children: ReactNode }) {
  ...
  function remover(id: number, corSelecionada?: Cor) {
    setItens((prev) =>
      prev.filter(
        (item) =>
          !(item.id === id && (item.corSelecionada?.nome || "") === (corSelecionada?.nome || ""))
      )
    );
  }

  function atualizarQuantidade(id: number, quantidade: number, corSelecionada?: Cor) {
    setItens((prev) =>
      prev.map((item) =>
        item.id === id && (item.corSelecionada?.nome || "") === (corSelecionada?.nome || "")
          ? { ...item, quantidade }
