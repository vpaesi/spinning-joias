...
export default function Carrinho() {
  ...
  const total = itens.reduce((acc, item) => acc + (item.produto.preco || 0) * item.quantidade, 0);
  ...
  function gerarMensagem() {
    ...
    itens.forEach((item, idx) => {
      ...
      msg += ` - Qtd: ${item.quantidade} - Valor: ${formatoDoPreco(item.produto.preco)}\n`;
    });
    msg += `\nTotal: ${formatoDoPreco(total)}\n\n`;
    msg += `Dados para envio:\nNome: ${form.nome}\nCPF: ${form.cpf}\nEndereço: ${form.endereco}\nCidade: ${form.cidade}\nUF: ${form.uf}\nCEP: ${form.cep}\n`;
    msg += `\nForma de pagamento: ${pagamento === "pix" ? "Pix" : "Transferência"}\n`;
    setMensagem(msg);
  }
      ...
      {itens.length === 0 ? (
        ...
        <>
                ...
                <tr key={idx} className="border-b border-gray-200">
                  ...
                  <td className="border-r border-gray-200">
                    <select
                      ...
                      onChange={e => atualizarQuantidade(item.id, Number(e.target.value), item.corSelecionada)}
                    >
                      {[...Array(10)].map((_, i) => (
                  ...
                  <td className="border-r border-gray-200">{formatoDoPreco(item.produto.preco)}</td>
                  <td>
                    <button
          ...
          <div className="flex justify-between font-bold mb-4">
            <span>Total de produtos: {itens.reduce((acc, item) => acc + item.quantidade, 0)}</span>
          ...
          <form className="bg-gray-50 p-4 rounded mb-4 grid grid-cols-1 gap-2">
            <input className="border p-2 rounded" placeholder="Nome completo" value={form.nome} onChange={e => setForm(f => ({ ...f, nome: e.target.value }))} />
            <input className="border p-2 rounded" placeholder="CPF" value={form.cpf} onChange={e => setForm(f => ({ ...f, cpf: e.target.value }))} />
            <input className="border p-2 rounded" placeholder="Endereço" value={form.endereco} onChange={e => setForm(f => ({ ...f, endereco: e.target.value }))} />
            <input className="border p-2 rounded" placeholder="Cidade" value={form.cidade} onChange={e => setForm(f => ({ ...f, cidade: e.target.value }))} />
            <input className="border p-2 rounded" placeholder="UF" value={form.uf} onChange={e => setForm(f => ({ ...f, uf: e.target.value }))} />
            <input className="border p-2 rounded" placeholder="CEP" value={form.cep} onChange={e => setForm(f => ({ ...f, cep: e.target.value }))} />
          </form>
          <div className="mb-4">
            <label className="mr-4">
              <input type="radio" checked={pagamento === "pix"} onChange={() => setPagamento("pix")} /> Pix
            </label>
            <label>
              <input type="radio" checked={pagamento === "transferencia"} onChange={() => setPagamento("transferencia")} /> Transferência
            </label>
          </div>
          <button className="btn mb-2" onClick={gerarMensagem}>Gerar mensagem automática</button>
          <textarea
          ...
          <div className="text-sm text-gray-600 mb-8">
            ...
            <a href={dadosLoja.socialMedia.whats} className="text-blue-500 underline" target="_blank" rel="noopener noreferrer">
